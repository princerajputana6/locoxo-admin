...
const Login = ({setToken, setUserRole, setUserData}) => {
  ...
  return (
    <div className='min-h-screen flex items-center justify-center w-full bg-white/5'>
        <div className='glass border border-white/10 rounded-xl shadow-lg p-10 max-w-md w-full'>
            ...
            <form onSubmit={onSubmitHandler}>
                <div className='mb-4 min-w-72'>
                    <p className='text-sm font-bold text-fg mb-2 uppercase tracking-wide'>Email Address</p>
                    <input onChange={(e)=>setEmail(e.target.value)} value={email} className='w-full px-4 py-3 border border-white/10 rounded-lg focus:border-gray-400 focus:ring-2 focus:ring-gray-100 outline-none transition-all' type="email" placeholder='admin@locoxo.com' required />
                </div>
                <div className='mb-6 min-w-72'>
                    <p className='text-sm font-bold text-fg mb-2 uppercase tracking-wide'>Password</p>
                    <input onChange={(e)=>setPassword(e.target.value)} value={password} className='w-full px-4 py-3 border border-white/10 rounded-lg focus:border-gray-400 focus:ring-2 focus:ring-gray-100 outline-none transition-all' type="password" placeholder='Enter your password' required />
                </div>
                <button className='w-full py-3 px-4 text-white font-semibold bg-surface-3 hover:brightness-110 transition-colors rounded-lg shadow-md' type="submit">Login</button>
            </form>
        </div>
    </div>
  )
